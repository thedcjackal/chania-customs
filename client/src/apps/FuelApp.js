...
const SettingsManager = () => {
    ...
    useEffect(() => { axios.get(`${API_URL}/admin/settings`).then(res => setSettings(res.data)); }, []); 
    if(!settings) return null; 
    const save = (s) => { setSettings(s); axios.post(`${API_URL}/admin/settings`, s); }; 
...
const DailyReport = ({ user }) => {
    ...
    const loadRes = useCallback(async () => { 
        const res = await axios.get(`${API_URL}/reservations?date=${date}`); 
        setReservations(res.data.sort((a,b) => b.location.x - a.location.x).map((r,i) => ({...r, sn: i+1}))); 
    }, [date]); 

    useEffect(() => { 
        axios.get(`${API_URL}/admin/employees`).then(res => setEmployees(res.data)); 
        axios.get(`${API_URL}/admin/reference`).then(res => setRefs(res.data)); 
        loadRes(); 
        axios.get(`${API_URL}/daily_status?date=${date}`).then(res => setIsFinalized(res.data.finalized)); 
    }, [date, loadRes]); 

    const handleAssign = async (id, name) => { await axios.put(`${API_URL}/reservations`, { id, role: 'admin', updates: { assigned_employee: name } }); loadRes(); }; 
    const toggleFinalize = async () => { try { const ns = !isFinalized; await axios.post(`${API_URL}/daily_status`, { date, finalized: ns }); setIsFinalized(ns); } catch (err) { alert("Error"); } }; 
    const runAssign = async () => { if (assignMode === 'single') { if (!singleEmp) return alert("Select Employee"); for (const r of reservations) await axios.put(`${API_URL}/reservations`, { id: r.id, role: 'admin', updates: { assigned_employee: singleEmp } }); } else if (assignMode === 'split') { if (!empA || !empB) return alert("Select 2 Employees"); const groups = {}; reservations.forEach(r => { const k = `${r.supply_company||'U'}|${r.fuel_type||'U'}`; if (!groups[k]) groups[k] = []; groups[k].push(r); }); let maxKey = null; let maxSize = -1; Object.keys(groups).forEach(k => { if (groups[k].length > maxSize) { maxSize = groups[k].length; maxKey = k; } }); for (const r of reservations) { const k = `${r.supply_company||'U'}|${r.fuel_type||'U'}`; const emp = (k === maxKey ? empA : empB); await axios.put(`${API_URL}/reservations`, { id: r.id, role: 'admin', updates: { assigned_employee: emp } }); } } setAssignMode(''); loadRes(); }; 
    const toggleDebt = async (r, action) => { const newFlags = action === 'remove' ? r.flags.filter(f => f !== 'Οφειλή') : [...r.flags, 'Οφειλή']; await axios.put(`${API_URL}/reservations`, { id: r.id, role: 'admin', updates: { flags: newFlags } }); setViewRes({...viewRes, flags: newFlags}); loadRes(); }; 
    const saveChanges = async () => { await axios.put(`${API_URL}/reservations`, { id: viewRes.id, role: 'admin', updates: viewRes }); setViewRes(null); loadRes(); }; 
...
const ReservationForm = ({ user, existing, onSuccess }) => {
    ...
    useEffect(() => { axios.get(`${API_URL}/admin/reference`).then(r => setRefs(r.data)); if (user.role !== 'user') axios.get(`${API_URL}/vessel_map`).then(res => setVesselMap(res.data)); }, [user.role]); 
...
const UserDashboard = ({ user }) => {
    ...
    const load = useCallback(async () => { 
        const res = await axios.get(`${API_URL}/reservations?company=${user.company}`); 
        setList(res.data); 
    }, [user.company]); 
    ...
    const del = async (r) => { if(window.confirm("Delete?")) { try { await axios.delete(`${API_URL}/reservations?id=${r.id}&role=user`); load(); } catch(e) { alert("Error"); } } }; 
